...
export default function Screen02({ navigation, route }) {
  ...
  const [listSearch, setListSearch] = useState(initialData);
  const [state, dispatch] = useReducer(jobReducer, initialData);
  ...
  const loadJobs = async () => {
    try {
      ...
      const data = jsonValue != null ? JSON.parse(jsonValue) : initialData;
      setListSearch(data);
    } catch (e) {
